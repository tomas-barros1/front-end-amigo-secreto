import type { LoginRequestDTO, LoginResponseDTO, RegisterRequestDTO } from "@/types/auth"
...
class ApiService {
  ...
  async addUserToGroup(groupId: string, userId: string): Promise<Group> {
    const response = await fetch(`${this.baseUrl}/groups/${groupId}/add/${userId}`, {
      method: "POST",
      headers: this.getHeaders(),
    })
    return this.handleResponse<Group>(response)
  ...
  async createDraw(groupId: string): Promise<Draw> {
    const response = await fetch(`${this.baseUrl}/draws/create?groupId=${groupId}`, {
      method: "POST",
      headers: this.getHeaders(),
    })
  ...
  async getMyFriend(groupId: string): Promise<FriendDrawDTO> {
    const response = await fetch(`${this.baseUrl}/draws/my-friend?groupId=${groupId}`, {
      headers: this.getHeaders(),
    })
